...
module.exports = {
  ...
  google: {
    ...
    serialize: passport.serializeUser(function(user, done) {
          DB.client.query("SELECT * FROM users WHERE fb_id = $1", [user.id], function(err, selectResult) {
            if (err) {
              ...
              DB.client.query("INSERT INTO users (fb_id, firstname, lastname, hometown, email) VALUES ($1, $2, $3, $4, $5) RETURNING *", [user.id, user.name.givenName], function(err, insertResult) {
                if(err) {
                  done(null, err);
                } else {
                  done(null, insertResult.rows[0]);
    ...
    strategy: passport.use(new GoogleStrategy({
      clientID: '928858731855-h2giaf6d01rm1l00u5dvqcrtgb90ju2u.apps.googleusercontent.com',
      clientSecret: 'D9ce0c1MIa_jTDoggRBK7QXG',
